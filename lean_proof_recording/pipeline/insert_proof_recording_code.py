...
from lean_proof_recording.modifier import LeanModifier
from lean_proof_recording.tokenizer import LeanFile, TokenType
...
class ModifyInterativeParameters:
    ...
    def extract_parser(param_expr: AST.Expr, param_ix: int, start: int) -> InteractiveParameter:
        ...
        for i in range(start, len(parts)):
            ...
            return InteractiveParameter(
                pos=Position(
                    parts[0].line, parts[0].column, parts[-1].end_line, parts[-1].end_column
                ),
                command="parse",
                param_ix=param_ix,
                parser_pos=Position(
                    parts[i].line, parts[i].column, parts[-1].end_line, parts[-1].end_column
                ),
            )
    ...
    def modify_def(
        ...
        for param in iparams:
            s += lean_file.slice_string(pos[0], pos[1], param.pos.line, param.pos.column)
    ...
    def monad_type(self, lean_file: LeanFile, ast: AST.DefLike) -> Optional[str]:
        if ast.signature.signature_type is not None:
            ...
            type_string = lean_file.slice_string(
                result_type.line, result_type.column, result_type.end_line, result_type.end_column
            ).strip()
