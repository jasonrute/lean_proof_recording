...
class LeanModifier:
    ...
    import_addition: Optional[List[str]]
    def __init__(self, lean_path: Path):
        self.lean_path = lean_path
        self.deletions = set()
        self.additions = {}
        self.end_addition = None
        self.import_addition = None
    ...
    def add_lines_after_imports(self, new_lines: str) -> None:
        """
        Add lines of code right after the imports

        new_lines:     New code lines to add to file.  
                       Single string with new lines.  Must end in a newline
        """
        assert new_lines.endswith("\n")
        self._insert_lines_after_imports(new_lines)
    ...
    def _insert_lines_after_imports(self, lines: str):
        assert self.import_addition is None, f"Can't make multiple additions to beginning of file."
        self.import_addition = lines[:-1].split("\n")
    ...
    def build_file(self, dryrun: bool = False, verbose=False):
        ...
        if dryrun:
            verbose=True
        if verbose: print(f'Modifications to file {self.lean_path}:')

        new_lines = []
        def append_new_lines(lines: List[str]):
            new_lines.append("--PR BEGIN CODE INSERT\n")
            if verbose: print("--PR BEGIN CODE INSERT")
            for new_line in lines:
                new_lines.append(new_line + '\n')
                if verbose: print(new_line)
            new_lines.append("--PR END CODE INSERT\n")
            if verbose: print("--PR END CODE INSERT")

        with open(self.lean_path, "r") as f:
            import_state = -1 # -1 means before imports, 0 means imports, 1 means done with imports
            for ix, line in enumerate(f):
                # begining of file changes (after imports)
                if import_state == -1 and line.startswith("import"):
                    import_state = 0
                elif import_state == 0 and not line.startswith("import"):
                    import_state = 1
                    if self.import_addition is not None:
                        append_new_lines(self.import_addition)

                # middle of file changes
                if ix in self.additions:
                    append_new_lines(self.additions[ix])
                if ix in self.deletions:
                    new_lines.append("--PR REMOVE LINE: " + line)
                    if verbose: print("--PR REMOVE LINE: " + line.rstrip())
                else:
                    new_lines.append(line)

            # end of file changes
            if self.end_addition is not None:
                append_new_lines(self.end_addition)
